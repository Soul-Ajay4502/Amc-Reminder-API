...
app.post('/upload', checkAuth, async (req, res) => {
    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            const queryGetImage = `SELECT dp_image FROM user_details WHERE user_id = ?`;
            ...
            if (existingFilename) {
                ...
                const existingImagePath = path.join('public', 'profile', existingFilename);
...
app.get('/userProfile', checkAuth, async (req, res) => {
    try {
        ...
        const imageUrl = `${req.protocol}://${req.get('host')}/profile/${filename}`;
        result.forEach(item => {
